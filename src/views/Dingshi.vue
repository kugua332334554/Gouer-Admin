  ...
  <div>
    ...
    <v-card>
      <v-data-table :headers="headers" :items="items" :loading="loading" items-per-page="30">
        ...
        <template #item.schedule_time="{ value }"><span v-if="value" class="font-weight-bold">{{ value }}</span><span v-else class="text-disabled">—</span></template>
        <template #item.schedule_days="{ value }">{{ value || '—' }}</template>
        <template #item.interval_minutes="{ value }">{{ value ? value + '分' : '—' }}</template>
        <template #item.content_text="{ value }">
          <v-tooltip location="bottom" max-width="500" v-if="value">
            <template #activator="{ props }">
              <span v-bind="props" class="text-truncate d-inline-block" style="max-width:220px">{{ value }}</span>
            </template>
    ...
    <v-dialog v-model="dialog" max-width="600">
      <v-card :title="isEdit ? '编辑定时消息' : '添加定时消息'">
        <v-card-text>
          <v-text-field label="群组ID" v-model.number="form.chat_id" type="number" density="compact" hide-details />
          <v-text-field label="时间 (HH:MM)" v-model="form.schedule_time" density="compact" hide-details class="mt-2" placeholder="08:00" />
          <v-text-field label="日期 (cron, *=每天)" v-model="form.schedule_days" density="compact" hide-details class="mt-2" />
          <v-text-field label="间隔(分钟, 0=关闭)" v-model.number="form.interval_minutes" type="number" density="compact" hide-details class="mt-2" />
          <v-textarea label="消息内容" v-model="form.content_text" rows="3" density="compact" hide-details class="mt-2" />
...
const form = ref({ id: 0, chat_id: 0, schedule_time: '', schedule_days: '*', interval_minutes: 0, content_text: '', buttons_text: '', status: true })

const headers = [
  { title: 'ID', key: 'id', width: 50 },
  { title: '群组', key: 'chat_id', width: 140 },
  { title: '时间', key: 'schedule_time', width: 70 },
  { title: '周期', key: 'schedule_days', width: 80 },
  { title: '间隔', key: 'interval_minutes', width: 60 },
  { title: '内容', key: 'content_text' },
...
async function load() {
  ...
  try { items.value = await api.get('/dingshi', { params }) } catch {} finally { loading.value = false }
}

function resetForm() {
  form.value = { id: 0, chat_id: 0, schedule_time: '', schedule_days: '*', interval_minutes: 0, content_text: '', buttons_text: '', status: true }
}
function openAdd() { resetForm(); isEdit.value = false; dialog.value = true }
function openEdit(item) {
  form.value = { ...item }
  if (item.interval_minutes === null) form.value.interval_minutes = 0
...
async function save() {
  ...
  try {
    if (isEdit.value) {
      const { id, chat_id, ...data } = form.value
      await api.put(`/dingshi/${id}`, data)
    } else {
      await api.post('/dingshi', form.value)
    }
    dialog.value = false; load()
  } catch (e) {} finally { saving.value = false }
}

async function del(id) { try { await api.delete(`/dingshi/${id}`); load() } catch {} }
